<template>
    <section class="w-[1440px] flex items-center justify-start gap-10 px-[100px] my-[115px]">
        <div class="judul w-[178px] h-[51px] bg-[#B9FF66] rounded-md text-center">
            <h2 class="font-space-grotesk text-[40px] font-medium leading-[51.04px] m-0">Services</h2>
        </div>
        <div class="isi w-[580px] ">
            <p class="font-space-grotesk text-[18px] font-normal leading-[22.97px] text-left m-0">At our digital marketing agency, we offer a range of services to help businesses
                grow and succeed online. These services include:</p>
        </div>
    </section>
</template>